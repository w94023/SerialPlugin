...
extern "C"
{
	...
	SERIALPORT_DECLSPEC void SetConnectionTimeout(const wchar_t* label, int timeout)
	{ 
		if (_manager.find(label) != _manager.end())
			_manager[label]->SetConnectionTimeout(timeout);
	}
	...
	SERIALPORT_DECLSPEC void CheckResources(const wchar_t* label, double* cpuUsage, int* memoryRemained)
	{
		if (_manager.find(label) != _manager.end()) {
			_manager[label]->CheckResources(cpuUsage, memoryRemained);
		}
	}

	SERIALPORT_DECLSPEC void Test(const wchar_t* label, double cpuUsage, int memoryRemained)
	{
		if (_manager.find(label) != _manager.end()) {
			_manager[label]->Test(cpuUsage, memoryRemained);
		}
	}

	SERIALPORT_DECLSPEC void SetPPSLimit(const wchar_t* label, int PPSLimit)
	{
		if (_manager.find(label) != _manager.end())
			_manager[label]->SetResourceLimit(PPS, PPSLimit);
	}

	SERIALPORT_DECLSPEC void SetCPULimit(const wchar_t* label, double CPULimit)
	{
		if (_manager.find(label) != _manager.end())
			_manager[label]->SetResourceLimit(CPU, CPULimit);
	}

	SERIALPORT_DECLSPEC void SetMemoryLimit(const wchar_t* label, int MemoryLimit)
	{
		if (_manager.find(label) != _manager.end())
			_manager[label]->SetResourceLimit(Memory, MemoryLimit);
	}
	...
	SERIALPORT_DECLSPEC int GetRecvData(const wchar_t* label, char* data, int byteSize)
	{
		if (_manager.find(label) != _manager.end())
			return _manager[label]->GetRecvData(data, byteSize);
		else 
			return 0;
	}
