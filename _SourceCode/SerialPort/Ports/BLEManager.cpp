...
BLEManager::BLEManager(LogManager& log, ConnectionConfig& config, DataManager& dataManager, CThreadManager& threadManager, std::function<void()> closeFunc) :
	PortManager(log, config, dataManager, threadManager, closeFunc), _scanner(log)
{
	threadManager.RegisterCallback(std::bind(&BLEManager::RecvThreadFunc, this, std::placeholders::_1));
