...
public:
	...
	void SetResourceLimit(ResourceConfig resourceConfig)
	{
		if (resourceConfig.useCPULimit    == 1) _cpuLimit    = resourceConfig.CPULimit;
		else                                    _cpuLimit    = -1;
		if (resourceConfig.useMemoryLimit == 1) _memoryLimit = resourceConfig.memoryLimit;
		else                                    _memoryLimit = -1;
	}
	...
	void ThreadFunc()
	{
		...
		while (true) {
			...
			if (!_stopDataThread && _isAlive) {
				...
				int elapsedTime = (int)std::chrono::duration_cast<std::chrono::milliseconds>(currentTimePoint - _threadWorkPoint).count();
